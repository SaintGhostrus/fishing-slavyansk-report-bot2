    ...
    try:
        msg = await bot.send_message(
            ...
            text="🎣 **Бот отчётов о рыбалке - Славянский район**\n\n"
                 ...
                 "• Автоматически пересылаются в основную тему чата\n"
                 "• Комментировать можно сразу в основной теме\n\n"
                 "⚠️ **Что нужно сделать:**\n\n"
                 ...
                 "Отчёт сразу появится в основной теме для обсуждения!",
...
async def send_report(cb: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        # 2. ДЕЛАЕМ FORWARD в ОСНОВНУЮ ТЕМУ чата (без thread_id)
        print(f"🔄 Делаем forward в основную тему чата...")
        
        # ОСНОВНАЯ ТЕМА ЧАТА - без указания message_thread_id
        ...
        print(f"✅ Forward сделан в основную тему, ID: {forwarded_msg.message_id}")
        ...
        # 5. Создаём кнопку "Отправить отчёт"
        ...
        try:
            ...
            print(f"✅ Кнопка '📤 Отправить отчёт' добавлена в тему {REPORT_THREAD_ID}, ID: {buttons_msg.message_id}")
        except Exception as e:
            print(f"❌ Ошибка при отправке кнопок: {e}")
        
        print(f"✅ Отчёт автоматически переслан в основную тему для комментариев")
        print(f"📝 Ссылка на отчёт в основной теме: https://t.me/c/{str(MAIN_CHAT_ID)[4:]}/{forwarded_msg.message_id}")
        ...
        success_msg = await bot.send_message(
            ...
            text="✅ Отчёт успешно опубликован!\n\n"
                 ...
                 f"💬 **Для комментариев:** основная тема чата\n\n"
                 f"📝 Отчёт автоматически переслан в основную тему для обсуждения.\n"
...
async def main():
    ...
    print(f"   Основная тема: без thread_id")
