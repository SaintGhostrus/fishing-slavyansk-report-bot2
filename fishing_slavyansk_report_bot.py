...
REPORT_THREAD_ID = 15708       # Тема для отчётов (как раньше)
    ...
    try:
        msg = await bot.send_message(
            ...
            text="🎣 **Бот отчётов о рыбалке - Славянский район**\n\n"
                 ...
                 "• Отчёты публикуются в тему 15708\n"
                 "• Кнопка «Комментировать» перешлёт отчёт в тему для обсуждения\n"
                 "• Вы дописываете комментарий и отправляете\n\n"
                 "⚠️ **Что нужно сделать:**\n\n"
                 ...
                 "4️⃣ Отправьте отчёт\n\n"
                 "📍 **Как комментировать:**\n"
                 "После публикации нажмите «Комментировать» под отчётом",
...
async def send_report(cb: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        report_message_id = sent_messages[0].message_id
        
        print(f"✅ Отчёт отправлен в тему {REPORT_THREAD_ID}, ID сообщения: {report_message_id}")
        
        # 2. Создаём ссылку для пересылки в тему для комментариев (1)
        ...
        # Формируем специальную ссылку Telegram для пересылки
        # Формат: t.me/c/chat_id/message_id?comment=thread_id
        comments_url = f"https://t.me/c/{chat_num}/{report_message_id}?comment={COMMENTS_THREAD_ID}"
        
        # Альтернативный вариант (может работать лучше):
        # comments_url = f"https://t.me/c/{chat_num}/{report_message_id}?thread={COMMENTS_THREAD_ID}"
        
        # 3. Получаем username бота
        bot_info = await bot.get_me()
        bot_username = bot_info.username
        
        # 4. Создаём кнопки под отчётом в теме 15708
        ...
        # 5. Отправляем сообщение с кнопками ПОД отчётом в той же теме
        ...
        print(f"✅ Кнопки добавлены под отчётом в теме {REPORT_THREAD_ID}")
        print(f"✅ Ссылка для комментирования: {comments_url}")
        print(f"💡 При нажатии «Комментировать» откроется пересылка этого отчёта в тему {COMMENTS_THREAD_ID}")
        
        # 6. Отправляем сообщение об успехе пользователю
        success_msg = await bot.send_message(
            chat_id=user.id,
            text="✅ Отчёт опубликован!\n\n"
                 f"📍 **Опубликовано в:** тема {REPORT_THREAD_ID}\n"
                 f"💬 **Чтобы прокомментировать:**\n"
                 f"1. Нажмите «Комментировать» под отчётом\n"
                 f"2. Telegram откроет пересылку в тему {COMMENTS_THREAD_ID}\n"
                 f"3. Допишите ваш комментарий и отправьте"
        )
...
async def main():
    ...
    print(f"📊 Настройки: Чат {MAIN_CHAT_ID}, Тема отчётов: {REPORT_THREAD_ID}, Тема комментариев: {COMMENTS_THREAD_ID}")
